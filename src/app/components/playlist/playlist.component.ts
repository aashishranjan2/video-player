...
export class PlaylistComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.renderer.listen(this.playListSelect.nativeElement, 'change', () => {
      ...
      this.playSelectedList(this.playListSelect.nativeElement.value);
    });
  }
  ...
  /**
   * @description playSelectedList() gets call when new playList has been selected and
   * emit to parent(video-player-component) with listName
   * @param <string> listName
   */
  playSelectedList(listName: string): void {
   ...
   */
  playVideoFromPlaylist(video: VideoModel): void {
